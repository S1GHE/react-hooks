...
export const ChatMessages = () => {
  ...
  const [sendIntervalId, setSendIntervalId] = useState<NodeJS.Timeout | null>(null);
  ...
  return (
    <div style={{ border: '1px solid red', padding: '20px', margin: '10px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <p>Количество подключений: <strong>{connectionCount}</strong></p>
        <p>Компонент перерендерился: <strong>{renderCount.current}</strong> раз</p>
        <p>Получено сообщений: <strong>{messages.length}</strong></p>
      </div>
